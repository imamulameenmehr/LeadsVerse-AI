...
export default function EditLeadPage() {
    ...
    useEffect(() => {
        const fetchLead = async () => {
            ...
            if (data) {
                ...
                setTotalReviews(data.total_reviews ? data.total_reviews.toString() : '')
                setClientType(data.client_type || '')
                setLeadSource(data.lead_source || '')
                setLeadScore(data.lead_score ? data.lead_score.toString() : '')
                ...
                setExpectedCloseDate(data.expected_close_date ? data.expected_close_date.split('T')[0] : '')
                setClientBudget(data.client_budget ? data.client_budget.toString() : '')
                setQuotedPrice(data.quoted_price ? data.quoted_price.toString() : '')
            }
            setFetching(false)
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (leadError) {
            setError(leadError?.message ?? 'Failed to update lead')
            setLoading(false)
            return
        }
    ...
    if (fetching) {
        return <div className="p-8 text-center text-zinc-500">Loading lead data...</div>
    }

    return (
        <div className="max-w-4xl mx-auto pb-10">
            <div className="mb-8">
                <h1 className="text-3xl font-bold tracking-tight uppercase">Edit Lead</h1>
                <p className="text-sm text-zinc-500 mt-2">Update the details for this lead below.</p>
            </div>
